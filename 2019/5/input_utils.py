...
def get_single_csv_input_as_ints(input_id: int, delimiter=','):
    file_name = get_input_file_name(input_id)
    print('Getting single csv row input as ints from ' + file_name)
    return convert_strings_to_ints(read_file_as_single_csv(file_name, delimiter))

def read_file_as_single_csv(file_name, delimiter=','):
    return read_file_as_string(file_name).split(delimiter)
