...
def get_input_as_list_of_chars(input_id: int):
    file_name = get_input_file_name(input_id)
    print('Getting input as list from ' + file_name)
    return read_file_as_list_of_chars(file_name)

def read_file_as_list_of_chars(file_name: str):
    return list(map(lambda x: list(x), open(file_name, 'r').read().splitlines()))
...
def get_input_as_csv_ints(input_id: int, delimiter=','):
    file_name = get_input_file_name(input_id)
    print('Getting input as csv ints from ' + file_name)
    return read_file_as_csv_ints(file_name, delimiter)

def read_file_as_csv_ints(file_name, delimiter=','):
    return list(map(lambda i: int(i), open(file_name, 'r').read().split(delimiter)))

sample_1_file_name = 'samples1.txt'
sample_2_file_name = 'samples2.txt'

def get_sample(sample_num: int):
    file_name = get_sample_file_name(sample_num)
    print('Getting samples as string from ' + file_name)
    return read_file_as_string(file_name)
...
def print_list_of_lists(list_of_lists):
    print('\n'.join(map(lambda x: ''.join(x), list_of_lists)))